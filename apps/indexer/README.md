...
| `GET /v1/people?query=<nickname or address>&limit=` | `{ items: [profile summary] }`; partial nicknames are case/accent insensitive. Duplicate names are returned as separate accounts. Query max 64 characters; default limit 20, max 100. Only inline public profile documents are decoded; no external URLs are fetched. |
