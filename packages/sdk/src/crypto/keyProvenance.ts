 ...
 */
import { addressToString } from "../ids.js";
import { bytesEqual, decode, fromBase64url } from "../encoding.js";
import { decodeBlockEvents, isEvent, type BlockLike } from "../events.js";
import { parseKeyPackageSet, type SealedKey } from "./audience.js";
import type { Deployment } from "../client/deployments.js";
import type { ProviderInterface } from "koilib";

export interface KeyProvenanceRef { author: string; audienceId: Uint8Array; epoch: number }
export interface SealedKeyEvidence { txId: string; sealedKey: string }
export type KeyProvenance =
  | { status: "verified"; recipients: string[] }
  | { status: "rejected"; reason: string }
  | { status: "unavailable"; reason: string };
export type KeyVerifier = (item: SealedKeyEvidence, ref: KeyProvenanceRef) => Promise<KeyProvenance>;
...
export async function verifySealedKeyProvenance(chain: ProvenanceChain, item: SealedKeyEvidence, ref: KeyProvenanceRef): Promise<KeyProvenance> {
  if (!item.txId) return { status: "rejected", reason: "the indexer named no transaction for this key" };
  let sealed: SealedKey;
  try {
    sealed = decode<SealedKey>("osp.envelope.sealed_key", fromBase64url(item.sealedKey));
  } catch {
